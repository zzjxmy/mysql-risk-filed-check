-- Structured archive seed converted from archiver/piwik/arch_log_link_visit_action.sh.
...
DELETE FROM archive_task WHERE name = 'piwik/arch_log_link_visit_action';

INSERT INTO archive_task (
  created_at, updated_at, name, task_mode, source_connection_id, dest_connection_id, status, remark
) VALUES (
  NOW(6), NOW(6), 'piwik/arch_log_link_visit_action', 'NORMAL', @source_connection_id, @dest_connection_id, 'DISABLED',
  'Converted from archiver/piwik/arch_log_link_visit_action.sh'
);
...
INSERT INTO archive_task_step (
  ...
  NOW(6), NOW(6), @task_id, 'log_link_visit_action archive old actions', 'ARCHIVE', 'dwd_analystic', 'log_link_visit_action', NULL,
  'legacy_dwd_analystic', 'log_link_visit_action', 'server_time < date_sub(now(),interval 1 month)',
