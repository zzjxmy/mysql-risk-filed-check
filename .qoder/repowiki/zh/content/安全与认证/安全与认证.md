...
- config：全局安全配置、CORS配置
- security：JWT工具、过滤器、用户详情服务
- controller/service：认证控制器与业务服务
- entity：用户实体与审计日志实体
- aspect：审计AOP切面
- util：AES加解密工具
...
- 权限装配：用户详情服务为每个用户装配“ROLE_前缀”的权限
...
章节来源
- [AuthService.java](file://backend/src/main/java/com/fieldcheck/service/AuthService.java#L30-L49)
- [SecurityConfig.java](file://backend/src/main/java/com/fieldcheck/config/SecurityConfig.java#L40-L42)
...
## 依赖分析
- 组件耦合
  - SecurityConfig依赖JwtAuthenticationFilter与CustomUserDetailsService
  - JwtAuthenticationFilter依赖JwtTokenProvider与CustomUserDetailsService
  - AuthService依赖AuthenticationManager、JwtTokenProvider、SysUserRepository、PasswordEncoder
...
## 结论
该平台采用JWT无状态认证、基于角色的权限控制、完善的审计日志与CORS配置，配合BCrypt与AES加密策略，构建了较为完整的安全体系。建议在生产环境中进一步收紧CORS范围、强化密钥管理、完善安全头策略，并持续监控与加固访问控制与日志分析能力。
